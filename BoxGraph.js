...
class BoxGraph {
  ...
  //These items don't belong in the loop    
     ...
     let xnlen = numlen(this.xmin,this.xmax,this.xnumbox); // maximum xnumber length in characters
        ...
        for(let i=0; i<floor(this.xnumbox+1); i++){
            ...
            if(this.xdecimals != 0){xn = xn.toFixed(this.xdecimals);} //does user want fixed decimal
  ...
  for(let i=0; i<this.ynumbox+1; i++){
     let tix = this.lowerLeft.y+this.m*i;
     line(this.lowerLeft.x,tix,this.lowerLeft.x+25*this.scale,tix);
     //if the numbers all have zero decimals, then omit the decimal
     let ynumber = str(this.ymin+i*(this.ymax-this.ymin)/this.ynumbox);
    strokeWeight(0);
     text(ynumber,this.lowerLeft.x-.0115*nlen*width,tix+.000*height);
     strokeWeight(3);
  } //end for
